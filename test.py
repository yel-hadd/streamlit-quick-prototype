...
spo2_median_line = go.Scatter(
    x=x, y=[spo2_median] * len(x), name="Median", mode="lines", line=dict(dash="dash")
)
...
heart_rate_mean_line = go.Scatter(
    x=x, y=[heart_rate_mean] * len(x), name="Mean", mode="lines", line=dict(dash="dash")
)
...
rr_interval_mean_line = go.Scatter(
    x=x,
    y=[rr_interval_mean] * len(x),
    name="Mean",
    mode="lines",
    line=dict(dash="dash"),
)
rr_interval_median_line = go.Scatter(
    x=x,
    y=[rr_interval_median] * len(x),
    name="Median",
    mode="lines",
    line=dict(dash="dash"),
)
...
rr_interval_fig = go.Figure(
    data=[rr_interval_chart, rr_interval_mean_line, rr_interval_median_line],
    layout=rr_interval_layout,
)
...
gyro_mean_line = go.Scatter(
    x=x, y=[gyro_mean] * len(x), name="Mean", mode="lines", line=dict(dash="dash")
)
gyro_median_line = go.Scatter(
    x=x, y=[gyro_median] * len(x), name="Median", mode="lines", line=dict(dash="dash")
)
...
gyro_fig = go.Figure(
    data=[gyro_chart, gyro_mean_line, gyro_median_line], layout=gyro_layout
)
